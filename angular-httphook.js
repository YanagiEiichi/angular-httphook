void function() {
  ...
  angular.module('httphook', ['httphook.provider'], ['$httpBackendProvider', 'httphookProvider', function($httpBackendProvider, httphookProvider) {
    ...
    var $handler = $httpBackendProvider.$get.splice(-1, 1, function () {
      ...
      return function(method, url, data, callback, headers, timeout, withCredentials, xRequestedWith) {
        httphookProvider.trigger({ method: method, url: url, data: data, callback: callback, headers: headers, timeout: timeout, withCredentials: withCredentials, 'X-Requested-With': xRequestedWith }, $delegate);
      };
    })[0];
  }]);
  ...
  angular.module('httphook.provider', []).provider('httphook', function() {
    ...
    this.trigger = function(request, $delegate) {
      ...
      hooks.solve('reqHandler', request.method, request.url, req, res, function() {
        ...
        $delegate(req.method, req.url, req.data, complete, req.headers, req.timeout, req.withCredentials, req["X-Requested-With"]);
      }, function() {
